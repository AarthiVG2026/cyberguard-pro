...
class SecurityAnalyzer:
    def __init__(self):
        ...
        self.trusted_domains = {
            ...
            'twitter.com', 'x.com', 'linkedin.com', 'github.com', 'stackoverflow.com',
            'wikipedia.org', 'paypal.com', 'chase.com', 'bankofamerica.com', 'wellsfargo.com',
            'citi.com', 'hsbc.com', 'barclays.com', 'standardchartered.com',
            'netflix.com', 'spotify.com', 'hulu.com', 'disney.com', 'disneyplus.com',
            'salesforce.com', 'oracle.com', 'ibm.com', 'dropbox.com', 'box.com',
            'zoom.us', 'slack.com', 'discord.com', 'telegram.org',
            'cloudflare.com', 'akamai.com', 'godaddy.com', 'bluehost.com',
            'adobe.com', 'dhl.com', 'fedex.com', 'ups.com', 'usps.com',
            'walmart.com', 'ebay.com', 'target.com', 'alibaba.com', 'aliexpress.com',
            'booking.com', 'airbnb.com', 'expedia.com', 'roblox.com', 'steampowered.com',
            'binance.com', 'coinbase.com', 'blockchain.com', 'metamask.io'
        }
        
        # Government and educational TLDs
        self.highly_trusted_tlds = {'.gov', '.mil', '.edu', '.ac.uk', '.gov.uk', '.gov.in'}
        
        # Suspicious TLDs
        self.suspicious_tlds = {
            '.tk', '.ml', '.ga', '.cf', '.click', '.download', '.loan',
            '.win', '.racing', '.stream', '.science', '.party', '.review', '.top', '.xyz', '.biz',
            '.gq', '.icu', '.monster', '.bid', '.dating', '.support', '.cam'
        }
        
        # Top domains for typosquatting detection
        self.highly_targeted_domains = {
            'google.com', 'facebook.com', 'amazon.com', 'apple.com', 'microsoft.com',
            'netflix.com', 'paypal.com', 'chase.com', 'bankofamerica.com', 'wellsfargo.com',
            'gmail.com', 'outlook.com', 'twitter.com', 'instagram.com', 'linkedin.com',
            'github.com', 'stackoverflow.com', 'wikipedia.org', 'adobe.com', 'zoom.us',
            'dhl.com', 'fedex.com', 'ups.com', 'booking.com', 'airbnb.com', 'roblox.com', 'steam.com',
            'binance.com', 'coinbase.com', 'blockchain.com', 'metamask.io', 'hsbc.com', 'citi.com',
            'barclays.com', 'visa.com', 'mastercard.com', 'amex.com', 'zellepay.com', 'cash.app', 
            'venmo.com', 'whatsapp.com', 'telegram.org', 'discord.com', 'spotify.com',
            'walmart.com', 'ebay.com', 'target.com', 'alibaba.com', 'aliexpress.com',
            'rakuten.com', 'nike.com', 'adidas.com', 'shein.com', 'appleid.com', 'office365.com'
        }
    ...
    def analyze_url_comprehensive(self, url):
        ...
        try:
            ...
            parsed = urlparse(url)
            # Ensure we have a valid domain after normalization
            domain = parsed.netloc.lower()
            if not domain and ':' in parsed.path:
                # Handle cases where path contains what should be a domain
            ...
            typo_result = self._check_typosquatting(clean_domain)
            result['detailed_analysis']['typosquatting'] = typo_result
            if typo_result['is_typosquatting']:
                security_score -= 35
                result['threat_types'].append('Typosquatting')
                result['mistake_description'] += f'This domain looks like a typosquatting attempt impersonating {typo_result["target"]}. '
                result['recommendations'].append(f'Carefully verify if you intended to visit {typo_result["target"]} instead of {clean_domain}.')

            # Check for high entropy (random looking domains)
            entropy = self._calculate_entropy(clean_domain.split('.')[0])
            result['detailed_analysis']['entropy'] = entropy
            if entropy > 4.5:
                security_score -= 10
                result['recommendations'].append('The domain name looks unusually random, which is common for generated malicious domains.')
            ...
            # Check domain length
            if len(clean_domain) > 25 and not is_trusted:
                security_score -= 10
                result['recommendations'].append('The domain name is unusually long, which is a common tactic in phishing to hide the full URL.')
    ...
    def _get_domain_age(self, domain):
        """Get domain age information using available WHOIS library"""
        ...
        try:
            ...
            if hasattr(python_whois, 'whois'):
                ...
                return {
                    ...
                    'expiry_date': None,
                    'registrar': None,
                    'status': 'Incompatible WHOIS library'
                }
            ...
            if creation_date:
                ...
                if isinstance(creation_date, datetime):
                    ...
                    # Use max(0, ...) to handle clock skew/new registrations
                    age_days = max(0, (now - creation_date).days)

                elif isinstance(creation_date, str):
                    # Try parsing date strings
                    for fmt in ('%Y-%m-%d %H:%M:%S', '%Y-%m-%d', '%d-%b-%Y', '%Y-%m-%dT%H:%M:%SZ'):
                        try:
    ...
    def _check_typosquatting(self, domain):
        """Check if a domain is a potential typosquatting attempt."""
        ...
        if len(parts) >= 2:
            ...
            return {'is_typosquatting': False, 'distance': 0, 'target': None}

        # If it's already in trusted, it's not typosquatting
        if self._is_trusted_domain(clean_domain):
            return {'is_typosquatting': False, 'distance': 0, 'target': None}

        # Get the main part of the domain (SLD)
        if len(parts) >= 2:
            sld = parts[-2]
            target_tld = parts[-1]
        else:
        ...
        sld_parts = sld.split('-')
        look_alikes = {'0': 'o', '1': 'l', '5': 's', '3': 'e', '8': 'b', 'q': 'g'}
        
        for sub_part in sld_parts:
            if not sub_part: continue
            
            normalized_sub = "".join(look_alikes.get(c, c) for c in sub_part)

            for trusted in self.highly_targeted_domains:
                trusted_sld = trusted.split('.')[0]
                
                # 1. Direct match or look-alike
                if sub_part == trusted_sld or normalized_sub == trusted_sld:
                    return {'is_typosquatting': True, 'distance': 0, 'target': trusted}
                    
                # 2. Character repetition 
                if len(sub_part) == len(trusted_sld) + 1:
                    for i in range(len(sub_part)):
                        if sub_part[:i] + sub_part[i+1:] == trusted_sld:
                            return {'is_typosquatting': True, 'distance': 1, 'target': trusted}
                
                # 3. Levenshtein distance
                distance = self._calculate_levenshtein_distance(sub_part, trusted_sld)
                threshold = 1 if len(trusted_sld) <= 5 else 2
                if 0 < distance <= threshold:
                    return {'is_typosquatting': True, 'distance': distance, 'target': trusted}

        return {'is_typosquatting': False, 'distance': 0, 'target': None}

    def _check_phishing(self, url, domain):
        """Enhanced check for phishing indicators"""
        url_lower = url.lower()
        ...
        for indicator in self.phishing_indicators:
            if indicator in url_lower:
                suspicious = True
                indicators_found.append(f'keyword_{indicator}')
        ...
        if '@' in url:
            suspicious = True
            indicators_found.append('at_symbol_usage')
            
        if domain.count('-') > 2:
            suspicious = True
            indicators_found.append('excessive_hyphens')

        # Check for suspicious TLDs in the middle of the URL (e.g., google.com.tk/...)
        path = urlparse(url).path
        for tld in self.suspicious_tlds:
            if tld in path:
                suspicious = True
                indicators_found.append(f'concealed_tld_{tld}')

        # Check for multiple dots in path (common in phishing)
        if path.count('.') > 3:
            suspicious = True
            indicators_found.append('path_dots_abuse')
        ...
        if suspicious:
            return {
                ...
                'suspicious': True,
                'indicators': indicators_found,
                'details': f'Suspicious patterns detected: {", ".join(indicators_found)}'
            }
        else:
            return {
                'status': 'Clean', 
                'suspicious': False,
                'indicators': [],
                'details': 'No obvious phishing indicators detected.'
            }
