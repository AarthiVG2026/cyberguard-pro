...
from core.extensions import db  # ← import db from extensions to avoid circular import

from models.database import User, URLCheck, AIScan, PasswordCheck, GeneratedPassword, SurveyResponse, PersonalPasswordGeneration, SecurityReport
from services.common.security_analyzer import SecurityAnalyzer
from services.common.report_generator import ReportGenerator
from datetime import datetime, timedelta
from models.database import get_current_time
from flask_bcrypt import Bcrypt

import io
import csv
# MULTI-AGENT ORCHESTRATOR (Project Core)
from services.orchestrator import orchestrator
import json
import logging

# Instantiate services to avoid NameError and ensure global availability
...
def sanitize_url(url):
    """Basic input sanitization for enterprise safety."""
    return url.strip()

# Error handlers (registered on blueprint, applied app-wide)
...
def index():
    try:
        # Get stats for the home page cards
        stats = {
            'total_users': User.query.count(),
            'total_url_checks': URLCheck.query.count(),
            'total_ai_scans': AIScan.query.count(), # Promoted metric
        ...
        logger.error(f"Error getting stats on index: {str(e)}")
...
@web_bp.route('/cyber-threats')
def cyber_threats():
    return render_template('cyber_threats.html')

@web_bp.route('/common-measures')
def common_measures():
    return render_template('common_measures.html')

@web_bp.route('/url-checker', methods=['GET', 'POST'])
def url_checker():
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                db.session.rollback()
                logger.warning(f"Database logging failed for URL check: {db_err}")
...
@web_bp.route('/password-checker', methods=['GET', 'POST'])
    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                db.session.rollback()
                logger.warning(f"Database logging failed for password check: {db_err}")
...
# ...existing code...
@web_bp.route('/password-generator', methods=['GET', 'POST'])
...
@web_bp.route('/personal-password', methods=['GET', 'POST'])
        ...
        try:
            ...
            if current_user.is_authenticated:
                ...
                try:
                    ...
                    db.session.rollback()
                    logger.warning(f"Database logging failed for personal passwords: {db_err}")
...
@web_bp.route('/download-report/<report_type>/<int:check_id>')
@web_bp.route('/download-report/<report_type>/<int:check_id>/<format>')
...
@web_bp.route('/admin/download-users')
@web_bp.route('/admin/download-users/<format>')
@login_required
...
@web_bp.route('/admin/download-activity')
@web_bp.route('/admin/download-activity/<format>')
@login_required
...
@web_bp.route('/admin/user-activity/<int:user_id>')
@login_required
...
@web_bp.route('/admin/toggle-user-status/<int:user_id>', methods=['POST'])
@login_required
...
@web_bp.route('/admin/feedback/<int:feedback_id>')
@login_required
...
@web_bp.route('/admin/feedback/<int:feedback_id>/mark-reviewed', methods=['POST'])
@login_required
...
@web_bp.route('/admin/download-feedback')
@web_bp.route('/admin/download-feedback/<format>')
@login_required
...
@web_bp.route('/admin-create', methods=['GET', 'POST'])
def admin_create():
    if request.method == 'POST':
        try:
            username = request.form.get('username', '').strip()
    ...
    try:
        if current_user.is_admin:
            ...
            stats = {
                ...
                'total_ai_scans': AIScan.query.count(), # AI PROMOTION
                'total_password_checks': PasswordCheck.query.count(),
                'total_generated_passwords': GeneratedPassword.query.count(),
            ...
            recent_ai_scans = AIScan.query.order_by(AIScan.checked_at.desc()).limit(10).all()
            ...
            return render_template('dashboard.html', 
                                 ...
                                 recent_ai_scans=recent_ai_scans,
                                 recent_feedback=survey_responses[:20])
        else:
            # User dashboard with history
            url_checks = URLCheck.query.filter_by(user_id=current_user.id).order_by(URLCheck.checked_at.desc()).all()
            ai_scans = AIScan.query.filter_by(user_id=current_user.id).order_by(AIScan.checked_at.desc()).all() # AI PROMOTION
            password_checks = PasswordCheck.query.filter_by(user_id=current_user.id).order_by(PasswordCheck.checked_at.desc()).all()
            generated_passwords = GeneratedPassword.query.filter_by(user_id=current_user.id).order_by(GeneratedPassword.generated_at.desc()).all()
...
@web_bp.route('/survey', methods=['GET', 'POST'])
def survey():
    if request.method == 'POST':
        try:
            # Get form data
...
# Note: ML/XAI services are orchestrated via services.orchestrator (not ml_service directly)

@web_bp.route('/ai-scanner')
def ai_scanner():
    """Render the AI Phishing Scanner UI."""
    return render_template('ai_scanner.html')

# ========================================================
# ENTERPRISE API ENDPOINTS (Cognizant Standard)
# ========================================================

@web_bp.route('/analyze-url', methods=['POST'])
def analyze_url_api():
    """
    POST /analyze-url
    Consolidated enterprise intelligence endpoint.
    """
    try:
        data = request.get_json()
        ...
        # 1. Orchestrate Multi-Agent Scan
        result = orchestrator.analyze(url)
        
        if not result.get('success'):
            return jsonify({'success': False, 'error': result.get('error')}), 500
            
        # 2. Log to Persistent Storage (Enterprise Audit Trail)
        try:
            new_scan = AIScan(
                ...
                ml_risk=result['agent_metrics']['ml_score'],
                rule_risk=result['agent_metrics']['rule_score'],
                features_json=json.dumps(result.get('features', {})),
                explanation=result['explanation'],
                full_details_json=json.dumps(result.get('details', {})),
            ...
            db.session.rollback()
            logger.error({"event": "db_log_failed", "url": url, "error": str(e)})

        return jsonify(result), 200

    except Exception as e:
        logger.error({"event": "api_exception", "endpoint": "/analyze-url", "error": str(e)})
        return jsonify({'success': False, 'error': 'Internal Server Error'}), 500

@web_bp.route('/check-password', methods=['POST'])
def check_password_api():
    """
    POST /check-password
    Advanced entropy and strength validation.
    """
    try:
        data = request.get_json()
        ...
        analysis = security_analyzer.analyze_password_strength(password)
        ...
        logger.error({"event": "api_exception", "endpoint": "/check-password", "error": str(e)})
        return jsonify({'success': False, 'error': 'Internal Server Error'}), 500
...
def get_history_api():
    ...
    try:
        ...
        scans = AIScan.query.filter_by(user_id=current_user.id)\
                            .order_by(AIScan.checked_at.desc())\
        ...
        history = [{
            ...
            'date': s.checked_at.strftime('%Y-%m-%d %H:%M:%S')
        ...
        logger.error({"event": "api_exception", "endpoint": "/history", "error": str(e)})
        return jsonify({'success': False, 'error': 'Internal Server Error'}), 500
